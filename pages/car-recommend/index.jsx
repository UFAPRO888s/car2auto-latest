          ...
          <div className="relative mx-auto max-w-lg divide-y-2 divide-gray-200 lg:max-w-7xl">
            <div>
              ...
              <p className="mt-3 text-xl text-gray-500 sm:mt-4">
                ...
                แนะนำเกี่ยวกับข้อมูลรถยนต์มือสอง จำนวน {props.dataMeta["count"]}{" "}
                เรื่อง แนะนำช่องทางติดตามข่าวสารรถยนต์ในวงการรถยนต์{" "}
                {props.dataMeta["name"]?.replace(/(<([^>]+)>)|&.+;/gm, " ")}
                ...
                >
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: postXS["excerpt"]["rendered"]
                        .slice(0, 250)
                        .replace(/(<([^>]+)>)|CARSOME|&.+;/gm, " "),
                    }}
                  ></div>
...
export async function getServerSideProps(context) {
  ...
  if (recom == "รีวิว") {
    recom = 439;
  } else if (recom == "ข่าวสาร") {
    recom = 175;
  } else if (recom == "รถกระบะ") {
    recom = 175;
  }
  const res = await fetch(
    `https://www.carsome.co.th/news/wp-json/wp/v2/posts?categories=${recom}&_embed&per_page=9&_fields=id,date_gmt,modified_gmt,slug,title,content,excerpt,categories,_links`
  );
  const data = await res.json();
  const resMeta = await fetch(
    `https://www.carsome.co.th/news/wp-json/wp/v2/categories/${recom}`
  );
